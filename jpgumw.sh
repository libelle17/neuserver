...
doumw() {
   ...
   blue_msg $1
   ext=${1##*\.}
   if test $ext = "jpg" -o $ext = "jpeg"; then 
     OCRRUMPF="$1"_ocr
     ...
     if ! test -f "$OCR"; then
      ...
      if ! test -f "$TMPTIF"; then
        echo -e "\nErzeuge $TMPTIF"
        ausf "convert \"$1\" \"$TMPTIF\"";
      fi
      if test -f "$TMPTIF"; then
         ausf "touch -r \"$1\" \"$TMPTIF\"";
         echo "Erzeuge " "$OCR"
         yell
...
# steht schon hier, weil sonst bei zwei gleichzeitig laufenden Instanzen das Datum nicht mehr angeglichen wird
     ...
     echo -e $BLAU_"$1"$SCHWARZ_ "endet nicht mit '$ROT_jpg$SCHWARZ_'"
   fi;
}
...
BILDANH="_bild"
if test -d "$1"; then
 FLS=$(find "$1" -maxdepth 1 -type f \( -name "*.jpg" -o -name "*.jpeg" \) -printf "%p|" 2>/dev/null); 
 case "$FLS" in "");; *) 
   for FL in $FLS; do 
    [ "$FL" ]&& doumw "$FL" "$2";
   done;;
 esac;
# for FILE in $1/*.jpg; do doumw $FILE $2 done
# for FILE in $1/*.jpeg; do doumw $FILE $2 done
elif test -f "$1"; then
 doumw "$1" "$2"
else
