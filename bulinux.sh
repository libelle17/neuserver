...
bu_db_erg() {
  ...
  for _db in $_bu_dbs; do
    ...
    while IFS='.' read -r _tbl _feld; do
      ...
      "SELECT CONCAT(c.table_name,'.',c.column_name) FROM information_schema.columns c WHERE c.table_schema='$_db' /* AND c.column_name REGEXP 'zeit|time|datum|^tag$|' */ AND c.data_type IN ('datetime','timestamp','date') ORDER BY c.table_name, c.ordinal_position;");
    if [ -n "$_col" ]; then
