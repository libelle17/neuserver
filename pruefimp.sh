...
#     echo "$TName"|hexdump -C
     ...
     init=$(basename "$file"|cut -d' ' -f1|sed "s/[-,.;_:']*$//;s/^\(.\{3\}\).*/\1/;s/[']/\\\\'/g")
#      stat "$file";
      sz=$(stat -c%s "$file")
      MT=$(stat -c%Y "$file")
      MTme=$(expr $MT - 1);
