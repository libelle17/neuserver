...
#  echo Name: $name, Datum: $datum, datediff: $datediff
}
...
#        echo "find $Vz -newermt $jahr-$monat-01 -not -newermt $datum -name "$name--????-??-??*.sql*" -print -quit;"
        ...
        befehl="find \"$Vz\" -newermt \"$jahr-$monat-01\" -not -newermt \"$datum\" -name \"$muende\" -print -quit";
