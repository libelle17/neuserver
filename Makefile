...
define priv_html
	...
	-@groff -mandoc -Thtml -v $(KR);EXC="$$$$?"; \
		echo 1:$(PGROFF);bp=$$$$(echo $(PGROFF));echo 2:$(PGROFF);echo bp:"$$$$bp;";for p in $$$$bp;do echo $$$$p;done;for p in $$$$bp;do { [ $$$$EXC -eq 0 ]&&which groff $(KR)&&$(SPR) $$$$p $(KR);}||{ sh configure inst _ $$$$p verbose;};done;:;
	-@rm -f $(1).html
