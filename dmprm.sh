...
commandline() {
  ...
  einzeln=;
  neu=;     # -n/--neu: bereits eingetragene PDF-Dateien neu einlesen
  letztneu=; # -ln/--letztneu: nur die Datei mit dem jüngsten dateidat neu einlesen
	while [ $# -gt 0 ]; do
   case "$1" in 
     -*|/*)
      para=${1#[-/]};
      case $para in
        e|-echt) obecht=1;;
        v|-verbose) verb=1;;
        h|'?'|-h|'-?'|/?|-hilfe) obhilfe=1;; # Achtung: das Fragezeichen würde expandiert
        -help) obhilfe=e;; # englische Hilfe
        nd|-neudd) neudb=1;;
        n|-neu) neu=1;;
        ln|-letztneu) letztneu=1;;
        *) 
          einzeln=1;
...
#		printf "obecht: $blau$obecht$reset\n";
    ...
    [ $neu ]&&printf "neu: $blau$neu$reset => bereits eingetragene PDF-Dateien werden gelöscht und neu eingelesen\n";
    [ $letztneu ]&&printf "letztneu: $blau$letztneu$reset => nur die Datei mit dem jüngsten dateidat wird neu eingelesen\n";
	fi;
} # commandline

# Gibt 0 (wahr) zurück, wenn die PDF-Datei echten Textinhalt enthält,
# also keine reine Bilddatei/OCR-Scan ist.
# Schwellwert: mehr als 200 Nicht-Leerzeichen im extrahierten Text.
hat_text() {
  _zeichen=$(pdftotext "$1" - 2>/dev/null | tr -d '[:space:]' | wc -c)
  [ "$_zeichen" -gt 200 ]
} # hat_text
...
if test -f "$pdf"; then
  ...
  BEGIN {
    zl=0;
    print "BEGIN;"
    if (!skipdel) print "DELETE FROM dmprm WHERE dateidat=STR_TO_DATE('\''" dateidat "'\'','\''%d.%m.%Y'\'');"
  }
...
if [ "$letztneu" ]; then
  # Datei mit dem jüngsten dateidat aus dmpeinl ermitteln und neu einlesen
  juengste=$(mariadb --defaults-extra-file=~/.mariadbpwd quelle -s -s \
    -e"SELECT e.Datei FROM dmpeinl e \
       INNER JOIN dmprm r ON r.einlID=e.ID \
       ORDER BY r.dateidat DESC, e.eingelesen DESC LIMIT 1;")
  if [ -z "$juengste" ]; then
    printf "${rot}Keine Einträge in der Datenbank gefunden.$reset\n";
  else
    [ $verb ]&&printf "Jüngste Datei laut dateidat: $blau$juengste$reset\n";
    mariadb --defaults-extra-file=~/.mariadbpwd quelle \
      -e"DELETE dmprm FROM dmprm INNER JOIN dmpeinl ON dmprm.einlID=dmpeinl.ID WHERE dmpeinl.Datei='$juengste';\
         DELETE FROM dmpeinl WHERE Datei='$juengste';";
    skipdel=1;
    qd="${juengste%.pdf}";
    auswert;
    skipdel=;
  ...
  # Einzeldatei: Verhalten wie bisher
  skipdel=;
  auswert;
else
  ...
  for pdf in "$qp"*.pdf; do
    ...
    if ! hat_text "$pdf"; then
      [ $verb ]&&printf "Überspringe $blau$pdf$reset (kein Textinhalt – vermutlich gescannt)\n";
      continue;
    fi

    # Prüfen ob diese PDF bereits in dmpeinl eingetragen ist
    bereits=$(mariadb --defaults-extra-file=~/.mariadbpwd quelle -s -s \
      -e"SELECT COUNT(*) FROM dmpeinl WHERE Datei='$pdf';")

    if [ "$bereits" -gt 0 ]; then
      if [ "$neu" ]; then
        # -n: alte Einträge dieser PDF gezielt löschen (TIF-Einträge bleiben erhalten)
        [ $verb ]&&printf "Datei $blau$pdf$reset bereits eingetragen – wird neu eingelesen (-n)\n";
        mariadb --defaults-extra-file=~/.mariadbpwd quelle \
          -e"DELETE dmprm FROM dmprm INNER JOIN dmpeinl ON dmprm.einlID=dmpeinl.ID WHERE dmpeinl.Datei='$pdf';\
             DELETE FROM dmpeinl WHERE Datei='$pdf';";
        # skipdel=1: awk soll kein dateidat-DELETE erzeugen, da wir schon per einlID gelöscht haben
        skipdel=1;
        qd="${pdf%.pdf}";
        auswert;
        skipdel=;
      else
        [ $verb ]&&printf "Überspringe $blau$pdf$reset (bereits eingetragen; -n zum erneuten Einlesen)\n";
      fi
    else
      skipdel=;
      qd="${pdf%.pdf}";
